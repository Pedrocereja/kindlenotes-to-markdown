...
def kindle2md(html_file):
    ...
    if dest.exists():
        return(f'O arquivo de destino "{dest}" já existe.')

    if splitext(source_name)[-1] != '.html':
        return('O arquivo deve ser do tipo HTML')
    ...
    try:
        ...
        return(f'Falha ao salvar arquivo: {e}')
    return(f'Arquivo salvo em: {dest_name}')